...
namespace ArcGIS_Poisonous
{
    public partial class FormQueryBySpatial : Form
    {
        ...
        private void FormQueryBySpatial_Load(object sender, EventArgs e)
        {
            ...
            // 将comboBoxSourceLayer控件的默认设置选项设置为第一种空间选择的方法
        ...
        public IGeometry GetFeatureLayerGeomtureLayerUnion(IFeatureLayer featureLayer)
        {
            // 定义IGeometry接口对象，存储每一个拓扑操作后得到的集合
            IGeometry geometry = null;
            // 使用ITopologicalOperator接口进行几何的拓扑操作
            ITopologicalOperator topologicalOperator;
            // 使用null作为查询过的过滤器得到图层中所有要素的游标
            ...
            while (feature != null)
            {
                ...
                if (geometry != null)
                {
                    //进行接口转换，使用当前几何体的ITopologicalOperator进行接口进行拓扑操作
                    topologicalOperator = geometry as ITopologicalOperator;
                    // 这些合并操作，将当期宿舍的几何进行Union，返回新的合并后的集合体
        ...
        private void SelectFeatureBySpatial() 
        {
            ...
            // 默认使用与查询的空间几何体为当前地图图源索要的韩剧几何体几个的集合
            spatialFilter.Geometry = GetFeatureLayerGeomtureLayerUnion
