    ...
    public partial class FormSelection : Form
    {
        ...
        private void FormSelection_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < currentMap.LayerCount; i++)
            {
                ...
                {
                    // 如果图层不是图层组类型的，则同样在TreeView控件中的根节点下添加节点
                    layerName = currentMap.get_Layer(i).Name;
                    featureLayer = currentMap.get_Layer(i) as IFeatureLayer;
                    if ((featureLayer as IFeatureSelection).SelectionSet.Count > 0)
                    {
                        // 新建一个TreeNode，将图层名称作为树节点的名称
                        treeNode = new TreeNode(layerName);
            ...
            labelMapSelectionCount.Text = "当前地图选择了" + currentMap.SelectionCount + "个要素";
        }
        ...
        private void treeViewLayers_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            ...
            // 通过接口转换，使用IFeatureSelecltion接口获取图层的选择集
            IFeatureSelection featureSelection = currentFeatureLayer as IFeatureSelection;
            // 通过ISelectionSet接口获取被选择的要素集合
            ISelectionSet selectionSet = featureSelection.SelectionSet;
            // 通过ISelectionSet接口中的count属性可以获取被选择要素的数量
            labelMapSelectionCount.Text = "当前图层选择了" + selectionSet.Count + "个要素";
            
            // 对当前图层要素的属性字段进行遍历，从而简历DataGridView中的列
            // 获得所有属性字段
            IFields fields = currentFeatureLayer.FeatureClass.Fields;
            // 通过遍历添加列，使用字段中的AlisaName作为DataGridView中显示的列名
            ...
            ICursor cursor;
            // 使用ISelectionSet接口方法的Search方法，使用null作为查询过滤器，cursor作为返回值，获取整个选择集
            selectionSet.Search(null, false, out cursor);
            // 进行接口转换，使用IFeatureCursor接口来获取选择集中的每个要素
            IFeatureCursor featureCursor = cursor as IFeatureCursor;
            // 获取IFeature接口的游标的第一个元素
            IFeature feature = featureCursor.NextFeature();
            // 定义string类型的数组，以添加DataGirdView中每行的数据
            string[] strs;
            // 当游标不为空时
            while (feature != null)
            {
        ...
        /// <summary>
        /// 在列表中点击某一行时发生，将主体的地图定位到该行所表示的要素范围
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void dataGridView_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            // 使用IFeatureClass接口的GetFeature方法，根据ObjectID获取该值
            IFeature feature = currentFeatureLayer.FeatureClass.GetFeature(objectID);
            // 定义新的IEnvelope接口对象，获取该要素的范围空间
